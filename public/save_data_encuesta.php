...
function guardar_respuestas() {
    ...
    if ($response->status == true) {
        wp_send_json(['status' => true, 'response' => $response->response]);
    } else {
        wp_send_json(['status' => false, 'response' => $response->response]);
    }
}

// Función para obtener las respuestas guardadas.
function obtener_respuestas() {
    $id_encuesta = $_GET['encuesta'];
    $user_id = get_current_user_id();
    $token = get_user_meta($user_id, 'tokensinapsisplatform', true);

    $body = ["id_encuesta" => $id_encuesta];
    $response = RfCoreCurl::curl('/api/encuesta/get_user_responses', 'POST', $token, $body);

    if ($response->status == true) {
        wp_send_json(['status' => true, 'respuestas' => $response->respuestas]);
    } else {
        wp_send_json(['status' => false, 'message' => 'No se encontraron respuestas.']);
    }
}
